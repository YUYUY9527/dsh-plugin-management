...
/** 非 registry 来源的 spec（本地链接 / git / tarball URL）：安装命令要原样复用它们。 */
const NON_REGISTRY_SPEC_RE = /^(link:|file:|workspace:|portal:|github:|git\+|git:|https?:)/i

/**
 * 拼一条「安装 / 升级该插件」的 dsh 命令，供面板逐行展示与复制。
 *
 * 为什么不是简单地 `add <name>@latest`：
 *   · 本地链接与 git/tarball 依赖用 registry 名重装会**换掉来源**，
 *     必须原样复用依赖里记的 spec 才是可复现的命令；
 *   · registry 包知道最新版就钉到最新版（等于升级），否则装最新。
 *
 * @param {{ name: string, spec: string, local?: boolean }} item 依赖项
 * @param {string | null} latest 最新版本（未检查/已最新时为 null）
 * @param {string} profile 目标 profile 名
 * @returns {string} 一条可直接粘贴执行的命令
 */
export function installCommandFor(item, latest, profile) {
  const prefix = `dsh plugin --profile ${profile ?? 'web'} add `
  if (item.local === true || NON_REGISTRY_SPEC_RE.test(String(item.spec ?? ''))) {
    return prefix + String(item.spec ?? item.name)
  }
  return latest !== null && latest !== undefined ? `${prefix}${item.name}@${latest}` : `${prefix}${item.name}`
}
 ...
 * @param {string} [profile] 目标 profile（用于拼每行的安装命令）
 * @returns {object} 前端行
 */
export function buildRow(item, info, profile = 'web') {
  const row = {
    name: item.name,
    spec: item.spec,
  ...
  row.installCommand = installCommandFor(item, row.latest, profile)
  return row
}
...
export function createInventory({
  ...
  async function inventory(request = {}) {
    ...
    const result = {
      ...
      checked: false,
      checkError: null,
      plugins: external.map((item) => buildRow(item, null, profile)).sort(byRank),
    ...
    if (entry !== null && args.check === true) {
      ...
      result.plugins = external
        .map((item) =>
          buildRow(item, Object.hasOwn(map, item.name) ? map[item.name] : null, profile),
        )
        .sort(byRank)
    }
    return result
  ...
  /**
   * 卸载外部插件（从 profile 的依赖里移除）。
   *
   * 安全约束（这里是破坏性操作，宁可拒绝也不要猜）：
   *   · **必须点名**：names 为空直接抛错，绝不存在「不带 names = 卸全部」的默认；
   *   · 只允许**该 profile 里真实存在**的依赖，且拒绝官方 `@deepseek-ai/*`
   *     （面板本来也只列外部包，这是给 HTTP/工具入口加的一道网）；
   *   · 卸载走 `dsh plugin --profile <p> remove …`，官方转发器会顺带把包名从
   *     `dsh.profile.bundles` 里摘掉（否则残留的 bundle 行会让下次启动报错）。
   *
   * @param {object} [request] 参数
   * @param {string} [request.profile] 目标 profile
   * @param {string[]} request.names 要卸载的包名（至少一个）
   * @returns {Promise<object>} 执行结果（含命令、退出码、输出尾部、未命中的包名）
   */
  async function uninstall(request = {}) {
    const args = request !== null && typeof request === 'object' ? request : {}
    const snapshot = await load(false)
    const profile = resolveProfile(snapshot, args.profile)
    const entry = snapshot.profiles.find((candidate) => candidate.name === profile) ?? null
    if (entry === null) throw new Error(`profile not found: ${profile}`)
    const requested = (Array.isArray(args.names) ? args.names : []).filter(
      (value) => isSafePackageName(value) && !isOfficialName(value),
    )
    if (requested.length === 0) {
      throw new Error('uninstall requires at least one external package name')
    }
    const present = requested.filter((value) =>
      entry.items.some((item) => item.name === value),
    )
    const unknown = requested.filter((value) => !present.includes(value))
    if (present.length === 0) {
      throw new Error(`not a dependency of profile ${profile}: ${requested.join(', ')}`)
    }
    const primary = `dsh plugin --profile ${profile} remove ${present.join(' ')}`
    let result = await run(primary, { workdir: entry.dir, timeoutMs: UPDATE_TIMEOUT_MS })
    let command = primary
    if (result.exitCode !== 0 && COMMAND_MISSING_RE.test(`${result.stdout}\n${result.stderr}`)) {
      command = `pnpm remove ${present.join(' ')}`
      result = await run(command, { workdir: entry.dir, timeoutMs: UPDATE_TIMEOUT_MS })
    }
    cache = null
    return {
