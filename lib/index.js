 ...
 *        POST <apiPath>/uninstall  { profile?, names }   ← 必须点名，没有「卸全部」
...
async function handleApi(service, apiPath, req, res, getToolStatus) {
  ...
  try {
    ...
    if (req.method === 'POST' && route === '/uninstall') {
      const body = await readJsonBody(req)
      // 卸载必须点名：绝不允许「不带 names = 卸全部」这种默认
      send(
        200,
  ...
  ctx.inject(['tools'], (toolsCtx) => {
    ...
    toolsCtx.effect(() => {
      ...
      void mount().catch((error) => {
        // ⚠️ 这个 catch 是必须的：mount 是 fire-and-forget 的 async 函数，
        // 若让它以 rejection 收场，Node 默认的 unhandled-rejection 策略会
        // **直接终止 dsh 进程**（这坑在开发探针里真实踩过：一次未捕获的
        // rejection 就把宿主进程带崩了）。这里还有 createToolSpec 这种
        // 不在内部 try 里的步骤，更不能留空。
        log(ctx, 'warn', `tool mount failed - ${error instanceof Error ? error.message : String(error)}`)
      })
      return () => {
        cancelled = true
...
export function apply(ctx, config = {}) {
  ...
  const service = createInventory({
    run: createRunner(ctx, {
      ...
      warn: (message) => log(ctx, 'warn', message),
    }),
    detectRunningProfile: () => detectRunningProfile(ctx),
    log: (message) => log(ctx, 'info', message),
  })
    ...
    webCtx.effect(
      () =>
        webCtx.webServer.register({
          ...
          path: apiPath,
          handler: (req, res) =>
            // 同 mount：这个 promise 的 rejection 绝不能逃逸（未捕获 rejection = 宿主进程终止）
            void handleApi(service, apiPath, req, res, () => ({ ...toolStatus })).catch((error) => {
              log(ctx, 'warn', `api handler failed - ${error instanceof Error ? error.message : String(error)}`)
            }),
        }),
      `${name}: api`,
    )
