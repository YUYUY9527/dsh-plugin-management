...
import { createToolSpec } from './tool.js'
 ...
 */
async function handleApi(service, apiPath, req, res) {
...
/**
 * 注册模型工具：优先用官方 defineTool（带参数校验），否则注册原始定义。
 * @param {import('@deepseek-ai/cordis').Context} ctx 插件上下文
 * @param {{ inventory: Function, update: Function }} service 清单服务
 * @returns {void}
 */
      ...
      const mount = async () => {
        ...
        if (defineTool !== null) {
          try {
            ...
            ctx.logger?.warn?.(
              `${name}: defineTool rejected the spec, registering it raw - ${
        ...
        try {
          ...
          ctx.logger?.warn?.(
            `${name}: tool registration failed - ${error instanceof Error ? error.message : String(error)}`,
          )
        }
      }
      void mount()
    ...
    webCtx.effect(
      () =>
        webCtx.webServer.register({
          kind: 'prefix',
          path: apiPath,
          handler: (req, res) => handleApi(service, apiPath, req, res),
        }),
      `${name}: api`,
    )
  })
