 ...
 *
 * 为什么不用 `@deepseek-ai/dsh-tools` 的 defineTool 直接写死：
 *   本插件是“外挂包”，必须能在自己的依赖没装全的情况下也活下来。
 *   所以默认走本地 JSON Schema 注册（tools 注册表接受的就是
 *   defineTool 产出的同一形状），运行时若 @deepseek-ai/dsh-tools 可解析，
 *   再由 index.js 用真正的 defineTool 包一层（附带参数校验），失败则回落。
 *
...
export const DESCRIPTION =
  'List and update external (non-official) plugins installed in a dsh profile. action="list" returns every external dependency of the profile: installed version, latest version, whether it is an active dsh.profile.bundles layer, and whether it is a registry package or a local link. action="update" runs `dsh plugin --profile <p> update [--latest] <names|all>` (pnpm fallback) and returns the command output; a package replaced in place is only loaded after dsh restarts.'
...
/**
 * 造工具定义（runnable 的原始形态，可直接交给 ctx.tools.register）。
...
export function createToolSpec(service) {
  return {
    ...
    parameters: parameterSpecToJsonSchema(PARAMETERS),
    output: {
      schema: { type: 'json' },
