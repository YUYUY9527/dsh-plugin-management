...
export const PARAMETERS = {
  action: {
    type: 'string',
    required: true,
    enum: ['list', 'update', 'uninstall'],
    description:
      'list reads the inventory; update installs newer versions; uninstall removes the named packages from the profile.',
  },
  profile: {
    type: 'string',
    description:
  ...
  names: {
    ...
    items: { type: 'string' },
    description:
      'Package names. For update: omit to update every external plugin of the profile. For uninstall: REQUIRED — there is no "remove all".',
  },
  latest: {
    type: 'boolean',
...
export const DESCRIPTION =
  'List, update and uninstall external (non-official) plugins installed in a dsh profile. action="list" returns every external dependency of the profile: installed version, latest version, whether it is an active dsh.profile.bundles layer, whether it is a registry package or a local link, and installCommand — a ready-to-run `dsh plugin --profile <p> add …` for exactly that package. action="update" runs `dsh plugin --profile <p> update [--latest] <names|all>` (pnpm fallback). action="uninstall" runs `dsh plugin --profile <p> remove <names>` and REQUIRES explicit names — there is no "remove all". Both mutating actions return the command output; a package replaced or removed in place is only loaded/unloaded after dsh restarts.'
...
export function normalizeArgs(args) {
  ...
  if (action !== 'list' && action !== 'update' && action !== 'uninstall') {
    throw new Error('action must be "list", "update" or "uninstall"')
  }
  ...
  // 卸载必须点名：在参数层就挡住「不带 names = 卸全部」这种最危险的调用形态
  if (action === 'uninstall' && (normalized.names === undefined || normalized.names.length === 0)) {
    throw new Error('uninstall requires at least one package name')
  }
...
export function createToolSpec(service) {
  return {
    ...
    async execute(rawArgs) {
      ...
      try {
        if (args.action === 'update') return await service.update(args)
        if (args.action === 'uninstall') return await service.uninstall(args)
        return await service.inventory({ profile: args.profile, check: args.check !== false })
      } catch (error) {
        return { ok: false, error: error instanceof Error ? error.message : String(error) }
      }
    },
